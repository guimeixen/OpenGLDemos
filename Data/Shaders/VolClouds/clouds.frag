...
uniform float forwardSilverLiningIntensity;
...
float SampleNoise(vec3 pos, float coverage, float cloudType, float height_0to1)
{
	...
	vec4 noise = textureLod(noiseTexture, pos, 0).rgba;
...
{
	pos.x = pos.x * 0.000045 + time * timeScale;
	pos.z = pos.z * 0.000045 - time * timeScale;
...
void main()
{
	...
	//color = mix(vec4(0.0), color, a);
	//vec4 rain = rain(dir);
	//color = color * (1.0 - rain.a) + rain;
//	color = mix(color, vec4(0.0), clamp(-dir.y * 20.0 + 0.9, 0.0, 1.0));
}