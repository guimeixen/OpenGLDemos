...
struct DockContext
{
	...
	void rootDock(const ImVec2& pos, const ImVec2& size)
	{
		...
		if (size.y < 0.0f)		// To prevent the docks from changing sizes and positions when minimizing and then maximizing
			return;
	...
	void load()
	{
		...
		if (fp)
		{
			...
			for (int i = 0; i < ival; i++)
			{
				...
				fscanf(fp, "%s %31[^\n]", str2, &lab[0]);		// Changed this line so the dock label can have spaces otherwise it would crash if we tried to load the layout with a label with spaces
